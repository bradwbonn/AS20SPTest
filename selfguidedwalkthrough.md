...
Default values are filled in for you, but most of them will need to be adjusted for your specific deployment needs:
  ...
  * **DesiredFleetInstances** = This is the number of concurrent streaming sessions that your fleet can support in a steady state.  This parameter may be adjusted later either manually, or automatically using Fleet Auto Scaling.  For now, set it to a "best guess" estimate of your application's required user concurrency.
